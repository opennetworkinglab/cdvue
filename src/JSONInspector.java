...
public class JSONInspector {
    ...
    public void toString(JSONObject jsonObject) {
        String className = (String) jsonObject.get("class name"); //gets the associated JavaClass's fully qualified name to get other values

        boolean hCA = (boolean) jsonObject.get(className + ":hc"); //whether the class has an @Component annotation
        boolean hSA = (boolean) jsonObject.get(className + ":hs"); //whether the class has an @Service annotation
        boolean iI = (boolean) jsonObject.get(className + ":ii"); //whether or not the JavaClass is an interface or not
        List<JavaAnnotation> jca = (List) jsonObject.get(className + ":ca"); //all JavaAnnotations for the class
        List<JavaField> jfa = (List) jsonObject.get(className + ":f"); //all JavaField's from this class that contain an @Reference annotation
        List<JavaClass> jic = (List) jsonObject.get(className + ":ic"); //all JavaClass's that the class implements
